...
RunTime_TypeDef R[3]; // always keep this as 6, as max is 6 for carding
  ...
  if(htim == &htim7){ //100ms timer for state sending to BT
	  ...
	  if (timer7Count % 5 == 0){
		  ...
		  if (S.runMode== RUN_ALL){
			  mcParams.currentDeliveryMMin = CalculateDeliverySpeed(R[0].presentRPM);
			  mcParams.currentMtrsRun += mcParams.currentDeliveryMMin/60.0 * 0.5;
		  }
	  }
...
int main(void)
{
  ...
  ReadALSettingsFromEeprom(&al_bt);
   uint16_t alValidFlag = EE_ReadInteger(AL_VALID_FLAG_ADDR);
   if ((alValidFlag != AL_VALID_FLAG_VALUE) || (CheckALSettings(&al_bt) == 0)) {
       LoadDefaultALSettings(&al_bt);
       // write defaults so next boot reads valid data
       WriteALSettingsIntoEeprom(&al_bt);
   }
