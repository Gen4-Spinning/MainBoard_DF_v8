...
void UART_IDLECallback(void){
	...
	if (cmdMsgRecieved == 0){
		...
		if (BT.BTmsgGood){
		      ...
		      // *** ADD — AL settings save from app
		            else if (BT.information == AL_SETTINGS_FROM_APP){    // 0x11
		                S.BT_dataOK = BT_MC_parse_ALSettings();
		                S.alSettingsSent = 0;
		                S.switchState = TO_SETTINGS;
		            }
		                ...
		                // *** ADD — calibration run reuses diagnostics state
		                else if (BT.subState == BT_AL_CALIBRATION) {
		                    D.motorID      = BT_BR;
		                    D.typeofTest   = OPENLOOP;
		                    D.targetDuty   = 450;          // ~30% PWM open-loop, avoids tracking error
		                    D.rampUpTime   = 5;
		                    D.rampDownTime = 5;
		                    D.runTime      = 1200;         // 5 minutes max — remove rampSteadyRunTime_s
		                    D.direction    = BT_DEFAULT_DIR;   // replace DEFAULT_DIR_COMMAND
		                    S.BT_dataOK    = 1;
		                    S.switchState  = TO_DIAGNOSTICS;
		                    D.stopTest     = 0;
		                    S.alCalibActive = 1;
		            ...
		            // *** ADD — AL settings request from app
		            else if (BT.information == REQ_AL_SETTINGS_FROM_APP){ // 0x13
		                S.switchState = TO_SETTINGS;
		            }
			  else if (BT.information == DIAGNOSTICS_FROM_APP){
			  ...
			  // *** ADD alongside existing TOGGLE_LOGGING block
			  else if (BT.information == TOGGLE_AL_FROM_APP) {
			      S.sensorToggleState   = BT.subState;   // 0=OFF, 1=ON
			      S.sensorToggleUpdated = 1;
			      HAL_UART_Transmit_IT(&huart1, (uint8_t*)SAVINGSUCCESS, 6);
			  }
